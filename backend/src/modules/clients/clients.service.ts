...
export class ClientsService {
  ...
  async create(dto: CreateClientDto, companyId: number): Promise<ClientView> {
    ...
    if (ice) {
      const existingIce = await this.prisma.client.findFirst({
        where: { companyId, ice },
      });
      if (existingIce) {
  ...
  async findAll(query: QueryClientDto, companyId: number): Promise<PaginatedResult<ClientView>> {
  ...
  async findStats(companyId: number): Promise<ClientStats> {
    const [total, actifs, inactifs, bloques] = await Promise.all([
      this.prisma.client.count({ where: { companyId } }),
      this.prisma.client.count({ where: { companyId, statut: ClientStatut.ACTIF } }),
      this.prisma.client.count({ where: { companyId, statut: ClientStatut.INACTIF } }),
      this.prisma.client.count({ where: { companyId, statut: ClientStatut.BLOQUE } }),
    ]);

    return { total, actifs, inactifs, bloques };
  }

  async findOne(id: number, companyId: number): Promise<ClientView> {
  ...
  async update(id: number, dto: UpdateClientDto, companyId: number): Promise<ClientView> {
    const existing = await this.prisma.client.findFirst({ where: { id, companyId } });
    if (!existing) {
      throw new NotFoundException(`Client #${id} introuvable`);
    }
  ...
  async remove(id: number, companyId: number): Promise<{ id: number }> {
    ...
    // Relation checks by company name & companyId (historical text match in Voyage & Facture)
